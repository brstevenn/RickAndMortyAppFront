...
export class Episodes extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      end: this.props.episodes.info.pages,
    });
  }
  ...
  render() {
    return (
      <div className="episodes">
        {this.props.episodes.newEpisodes.length > 0 ? (
          <h3 className="newEpisodesTitle">New Episodes</h3>
        ) : (
          <div className="display"></div>
        )}
        {this.props.episodes.newEpisodes &&
        this.props.episodes.newEpisodes.length > 0 ? (
          <div className="newEpisodesContain">
            <div className="episodesCard">
              {this.props.episodes.newEpisodes &&
                this.props.episodes.newEpisodes.map((item) => {
                  return (
                    <EpisodeCard
                      key={item.id}
                      id={item.id}
        ...
        {this.props.episodes.database &&
        this.props.episodes.database.length > 0 ? (
          <div className="newEpisodesContain">
            <div className="episodesCard">
              {this.props.episodes.database &&
                this.props.episodes.database.map((item) => {
                  return (
                    <EpisodeCard
                      key={item.id}
                      id={item.id}
        ...
        <div className="episodesCard">
          {this.props.episodes.results &&
            this.props.episodes.results.map((item) => {
              return (
                <EpisodeCard
                  key={item.id}
                  id={item.id}
