...
export class Characters extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      end: this.props.characters.info.pages,
    });
  }
  ...
  render() {
    return (
      <div className="characters">
        {this.props.characters.newCharacters.length > 0 ? (
          <h3 className="newCharactersTitle">New characters</h3>
        ) : (
          <div className="display"></div>
        )}
        {this.props.characters.newCharacters &&
        this.props.characters.newCharacters.length > 0 ? (
          <div className="newCharactersContain">
            <div className="charactersCard">
              {this.props.characters.newCharacters &&
                this.props.characters.newCharacters.map((item) => {
                  return (
                    <CharacterCard
                      key={item.id}
                      id={item.id}
        ...
        {this.props.characters.database &&
        this.props.characters.database.length > 0 ? (
          <div className="newCharactersContain">
            <div className="charactersCard">
              {this.props.characters.database &&
                this.props.characters.database.map((item) => {
                  return (
                    <CharacterCard
                      key={item.id}
                      id={item.id}
        ...
        <div className="charactersPageContain">
          ...
          <button
            className="characterPagePrevNav"
            id="prev"
          ...
          <button
            className="characterPageNextNav"
            id="next"
            onClick={this.handleClickSlide}
          >
            {">"}
        ...
        <div className="charactersCard">
          {this.props.characters.results &&
            this.props.characters.results.map((item) => {
              return (
                <CharacterCard
                  key={item.id}
                  id={item.id}
